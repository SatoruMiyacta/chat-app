...
const GroupProfile = () => {
  ...
  const getGroupMember = async () => {
    ...
    const groupMemberIdList = await getGroupMemberList(postId, true);
    ...
    saveGroupsMemberIdList(postId, groupMemberIdList);
  };
  ...
  const handleScroll = async () => {
    ...
    getGroupMemberList(postId, false).then((groupMemberIdList) => {
  ...
  const showEditButton = () => {
    if (groupAuthorId === userId) {
      return (
        <Button
          color="primary"
          onClick={() => navigate(`/group/${postId}/edit`)}
          variant="outlined"
          isFullWidth
      ...
      {!isLoading && (
        <div className={styles.container}>
          ...
          <div className={`${styles.buttonArea} flex alic fdrc inner`}>
            <Button
              color="primary"
              onClick={() => navigate(`/rooms/${postId}/message`)}
              variant="contained"
              isFullWidth
              size={isPcWindow ? 'medium' : 'small'}
            >
